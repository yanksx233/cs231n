...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        exp_pos_pairs = torch.exp(sim(z_k, z_k_N)/tau)
        exp_neg_pairs_k, exp_neg_pairs_k_N = 0, 0
        for i in range(2*N):
            if i != k:
                exp_neg_pairs_k += torch.exp(sim(out[i], z_k)/tau)
            if i != k+N:
                exp_neg_pairs_k_N += torch.exp(sim(out[i], z_k_N)/tau)

        total_loss += -torch.log(exp_pos_pairs/exp_neg_pairs_k) - torch.log(exp_pos_pairs/exp_neg_pairs_k_N)
...
def compute_sim_matrix(out):
    ...
    norm_z = torch.linalg.norm(out, dim=1, keepdim=True)  # (2N, 1)
    norm_i_j = torch.mm(norm_z, norm_z.T) # (2N, 2N)
    sim_matrix = torch.mm(out, out.T) / norm_i_j  # (2N, 2N)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    pos_pair = sim_matrix[range(N), range(N, 2*N)][:, None]  # (N, 1)
    # pos_pair = sim_positive_pairs(out_left, out_right)  # (N, 1)
    ...
    numerator = torch.exp(pos_pair / tau).to(device)  # (N, 1)

    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    
    # Step 4: Now that you have the numerator and denominator for all augmented samples, compute the total loss.
    loss = torch.log(numerator / denom[:N]) + torch.log(numerator / denom[N:])
