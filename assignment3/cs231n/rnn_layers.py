...
def rnn_backward(dh, cache):
    ...
    dx = np.zeros([N, T, D])  # x.shape
    dWx = np.zeros([D, H])
    dWh = np.zeros([H, H])
    db = np.zeros(H)

    dprev_h_t = np.zeros((N, H))
    for t in range(T-1, -1, -1):  # 不要原地操作dh
        dx[:, t, :], dprev_h_t, dWx_t, dWh_t, db_t = rnn_step_backward(dh[:, t, :] + dprev_h_t, cache[t])
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
...
def word_embedding_forward(x, W):
    ...
    - x: Integer array of shape (N, T) giving indices of words. Each element idx
      of x muxt be in the range 0 <= idx < V. 即x的每个元素值value位于区间[0, V)
    ...
    # An example of where this may be useful is for a color
    # lookup table where we want to map the values of an image
    # into RGB triples for display. The lookup table could
    # have a shape (nlookup, 3). Indexing such an array with
    # an image with shape (ny, nx) with dtype=np.uint8 
    # (or any integer type so long as values are with 
    # the bounds of the lookup table) will result in an 
    # array of shape (ny, nx, 3) where a triple of RGB 
    # values is associated with each pixel location.
...
def word_embedding_backward(dout, cache):
    ...
    x, W = cache
    dW = np.zeros(W.shape)
    np.add.at(dW, x, dout)  # 相当于 dW[x] += dout
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dnext_c += o * (1 - np.tanh(next_c)**2) * dnext_h  # 小心这一步，dnext_c还要接受从dnext_h回流过来的梯度
    dprev_c = dnext_c * f
    df = dnext_c * prev_c
    di = dnext_c * g
    dg = dnext_c * i
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache = {}
    for t in range(T):
        h[:, t+1, :], c_temp, cache[t] = lstm_step_forward(x[:, t, :], h[:, t, :], c_temp, Wx, Wh, b)
...
def lstm_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
        dx[:, t, :], dh_temp, dc, dWx_t, dWh_t, db_t = lstm_step_backward(dh[:, t, :]+dh_temp, dc, cache[t])
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
    dh0 = dh_temp
